...
def main():
    ap = argparse.ArgumentParser(
        description="CDH discovery: list Hive/Impala databases and tables with storage type. "
                    "Produces dependency_objects CSV (table_type, storage_format)."
    )
    ...
    if args.databases:
        ...
        print("[cdh_hive] Listing databases...", file=sys.stderr)
    ...
    print("[cdh_hive] Found {} databases".format(len(databases)), file=sys.stderr)
    ...
    for i, db in enumerate(databases):
        ...
        if (i + 1) % 50 == 0 or (i + 1) == len(databases):
            print("[cdh_hive] Listed {}/{} databases, {} tables so far".format(
                i + 1, len(databases), len(all_tables)), file=sys.stderr)

    print("[cdh_hive] Total tables found: {}".format(len(all_tables)), file=sys.stderr)
    ...
    if args.skip_describe:
        print("[cdh_hive] Writing table list (no DESCRIBE)...", file=sys.stderr)
        ...
        print("[cdh_hive] Wrote {} rows to {}".format(len(all_tables), args.out_objects),
              file=sys.stderr)
        return

    print("[cdh_hive] Describing {} tables with {} workers...".format(
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as ex:
        ...
        for fut in as_completed(futures):
            ...
            if done % 500 == 0 or done == len(all_tables):
                print("[cdh_hive] Described {}/{} tables (errors={})".format(
    ...
    print("[cdh_hive] Wrote {} rows to {}".format(len(results), args.out_objects),
          file=sys.stderr)

    print("[cdh_hive] Storage format distribution:", file=sys.stderr)
