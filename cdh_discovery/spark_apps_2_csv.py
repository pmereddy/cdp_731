...
a dependency_access CSV.

Spark 2.x SHS does NOT support offset-based pagination. This script uses
...
def fetch_one_window(session, url, min_date, max_date, limit):
    ...
    if not resp.ok:
        print("[cdh_spark] ERROR: HTTP {}: {}".format(resp.status_code, resp.text[:500]), file=sys.stderr)
        return []
...
def fetch_apps(session, base_url, since_date, until_date, limit, sleep_ms):
    ...
    if not since_date:
        ...
        for app in apps:
            ...
            if app_id and app_id not in seen_ids:
                seen_ids.add(app_id)
                all_apps.append(app)
        print("[cdh_spark] Fetched {} apps (no date range)".format(len(all_apps)), file=sys.stderr)
        return all_apps
    ...
    while day <= end:
        ...
        if new_count > 0 or day_count % 7 == 0:
            print("[cdh_spark] {} fetched={} new={} total={}".format(
                day_str, len(apps), new_count, len(all_apps)), file=sys.stderr)

        if len(apps) >= limit:
            print("[cdh_spark] WARNING: {} returned {} apps (= limit). Increase --limit if needed.".format(
                day_str, len(apps)), file=sys.stderr)
...
def main():
    ap = argparse.ArgumentParser(
        description="CDH discovery: export Spark History Server applications to dependency_access CSV."
    )
    ...
    print("[cdh_spark] Wrote {} rows to {}".format(len(apps), args.out_access), file=sys.stderr)
